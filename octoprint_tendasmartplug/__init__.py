...
class tendasmartplugPlugin(octoprint.plugin.SettingsPlugin,
	...
	def __init__(self):
		self._logger = logging.getLogger("octoprint.plugins.tendasmartplug")
		self._tendasmartplug_logger = logging.getLogger("octoprint.plugins.tendasmartplug.debug")
		self.abortTimeout = 0
	...
	def on_startup(self, host, port):
		...
		tendasmartplug_logging_handler = CleaningTimedRotatingFileHandler(
			self._settings.get_plugin_logfile_path(postfix="debug"), when="D", backupCount=3)
		tendasmartplug_logging_handler.setFormatter(logging.Formatter("[%(asctime)s] %(levelname)s: %(message)s"))
		tendasmartplug_logging_handler.setLevel(logging.DEBUG)

		self._tendasmartplug_logger.addHandler(tendasmartplug_logging_handler)
		self._tendasmartplug_logger.setLevel(
			logging.DEBUG if self._settings.get_boolean(["debug_logging"]) else logging.INFO)
		self._tendasmartplug_logger.propagate = False
	...
	def on_after_startup(self):
		self._logger.info("tendasmartplug loaded!")
		...
		self._tendasmartplug_logger.debug("abortTimeout: %s" % self.abortTimeout)

		self.powerOffWhenIdle = self._settings.get_boolean(["powerOffWhenIdle"])
		self._tendasmartplug_logger.debug("powerOffWhenIdle: %s" % self.powerOffWhenIdle)

		self.idleTimeout = self._settings.get_int(["idleTimeout"])
		self._tendasmartplug_logger.debug("idleTimeout: %s" % self.idleTimeout)
		self.idleIgnoreCommands = self._settings.get(["idleIgnoreCommands"])
		self._idleIgnoreCommandsArray = self.idleIgnoreCommands.split(',')
		self._tendasmartplug_logger.debug("idleIgnoreCommands: %s" % self.idleIgnoreCommands)
		self.idleTimeoutWaitTemp = self._settings.get_int(["idleTimeoutWaitTemp"])
		self._tendasmartplug_logger.debug("idleTimeoutWaitTemp: %s" % self.idleTimeoutWaitTemp)
		if self._settings.get_boolean(["event_on_startup_monitoring"]) is True:
			self._tendasmartplug_logger.debug("powering on due to startup.")
			for plug in self._settings.get(['arrSmartplugs']):
				if plug["event_on_startup"] is True:
					self._tendasmartplug_logger.debug("powering on %s due to startup." % (plug["ip"]))
					...
					if response.get("currentState", False) == "on":
						...
						self._tendasmartplug_logger.debug("powering on %s during startup failed." % (plug["ip"]))
		...
		if self._settings.getBoolean(["event_on_shutdown_monitoring"]):
			for plug in self._settings.get(['arrSmartplugs']):
				if plug["event_on_shutdown"] is True:
					self._tendasmartplug_logger.debug("powering off %s due to shutdown event." % plug["ip"])
	...
	def on_settings_save(self, data):
		...
		if self.powerOffWhenIdle == True:
			self._tendasmartplug_logger.debug("Settings saved, Automatic Power Off Enabled, starting idle timer...")
			self._reset_idle_timer()
		...
		if old_debug_logging != new_debug_logging:
			if new_debug_logging:
				self._tendasmartplug_logger.setLevel(logging.DEBUG)
			else:
				self._tendasmartplug_logger.setLevel(logging.INFO)
	...
	def on_settings_migrate(self, target, current=None):
		if current is None or current < 5:
			...
			self._tendasmartplug_logger.debug("Resetting arrSmartplugs for tendasmartplug settings.")
			...
			self._tendasmartplug_logger.info("Updating plug array, converting")
			self._tendasmartplug_logger.info(self._settings.get(['arrSmartplugs']))
			self._tendasmartplug_logger.info("to")
			self._tendasmartplug_logger.info(arrSmartplugs_new)
	...
	def get_template_configs(self):
		templates_to_load = [dict(type="navbar", custom_bindings=True), dict(type="settings", custom_bindings=True),
							 dict(type="sidebar", icon="plug", custom_bindings=True,
								  ...
								  template="tendasmartplug_sidebar.jinja2",
								  template_header="tendasmartplug_sidebar_header.jinja2"),
							 dict(type="tab", custom_bindings=True, data_bind="visible: show_sidebar()",
								  template="tendasmartplug_tab.jinja2")]
	...
	def on_print_progress(self, storage, path, progress):
		if self._settings.get_boolean(["progress_polling"]) is False:
			return
		self._tendasmartplug_logger.debug("Checking statuses during print progress (%s)." % progress)
		...
		if self.powerOffWhenIdle is True and not (self._skipIdleTimer is True):
			self._tendasmartplug_logger.debug("Resetting idle timer during print progress (%s)..." % progress)
			self._waitForHeaters = False
			self._reset_idle_timer()
	...
	def turn_on(self, plugip):
		self._tendasmartplug_logger.debug("Turning on %s." % plugip)
		plug = self.plug_search(self._settings.get(["arrSmartplugs"]), "ip", plugip)
		self._tendasmartplug_logger.debug(plug)
		if "/" in plugip:
			plug_ip, plug_num = plugip.split("/")
		else:
			plug_ip = plugip
			plug_num = 0
		self.sendCommand("setSta", plug_ip, json={"status": 1})

	def turn_off(self, plugip):
		timenow = datetime.now()
		self._tendasmartplug_logger.debug("Turning off %s." % plugip)
		self._tendasmartplug_logger.info("Turning off %s at %s" % (plugip, timenow))
		plug = self.plug_search(self._settings.get(["arrSmartplugs"]), "ip", plugip)
		self._tendasmartplug_logger.debug(plug)
		if "/" in plugip:
			plug_ip, plug_num = plugip.split("/")
			...
			plug_ip = plugip
			plug_num = 0
		self.send_command("setSta", plug_ip, json={"status": 0})
	...
	def check_status(self, plugip):
		self._tendasmartplug_logger.debug("Checking status of %s." % plugip)
		if plugip != "":
			r = self.sendCommand("getSta", plugip)
			result = r.json()
			return dict(currentState=result, ip=plugip, json=result)
	...
	def on_api_get(self, request):
		self._tendasmartplug_logger.debug(request.args)
		if request.args.get("checkStatus"):
			response = self.check_status(request.args.get("checkStatus"))
			return flask.jsonify(response)

	def on_api_command(self, command, data):
		if not Permissions.PLUGIN_tendasmartplug_CONTROL.can():
		...
		if command == 'turnOn':
			...
			db.close()
			self._tendasmartplug_logger.debug(response)
		# SELECT * FROM energy_data WHERE ip = '192.168.0.102' LIMIT 0,30
			...
			for plug in self._settings.get(["arrSmartplugs"]):
				if plug["useCountdownRules"] and int(plug["countdownOffDelay"]) > 0:
					...
					self._tendasmartplug_logger.debug("Cleared countdown rules for %s" % plug["ip"])
			self._tendasmartplug_logger.debug("Power off aborted.")
			self._tendasmartplug_logger.debug("Restarting idle timer.")
			self._reset_idle_timer()
		...
		if command == "enableAutomaticShutdown" or command == "disableAutomaticShutdown":
			self._tendasmartplug_logger.debug("Automatic power off setting changed: %s" % self.powerOffWhenIdle)
	...
	def on_event(self, event, payload):
		...
		if event == Events.STARTUP and self._settings.get_boolean(["event_on_startup_monitoring"]) is True:
			self._tendasmartplug_logger.debug("powering on due to %s event." % event)
			for plug in self._settings.get(['arrSmartplugs']):
				if plug["event_on_startup"] is True:
					self._tendasmartplug_logger.debug("powering on %s due to %s event." % (plug["ip"], event))
					response = self.turn_on(plug["ip"])
					if response["currentState"] == "on":
						self._plugin_manager.send_plugin_message(self._identifier, response)
		# Error Event
		if event == Events.ERROR and self._settings.getBoolean(["event_on_error_monitoring"]) is True:
			self._tendasmartplug_logger.debug("powering off due to %s event." % event)
			for plug in self._settings.get(['arrSmartplugs']):
				if plug["event_on_error"] is True:
					self._tendasmartplug_logger.debug("powering off %s due to %s event." % (plug["ip"], event))
		...
		if event == Events.CLIENT_OPENED:
			if self._settings.get_boolean(["powerOffWhenIdle"]):
				self._tendasmartplug_logger.debug("resetting idle timer due to %s event." % event)
				self._reset_idle_timer()
			self._plugin_manager.send_plugin_message(self._identifier, dict(powerOffWhenIdle=self.powerOffWhenIdle, type="timeout", timeout_value=self._timeout_value))
			return
		# Cancelled Print Interpreted Event
		if event == Events.PRINT_FAILED and not self._printer.is_closed_or_error():
			self._tendasmartplug_logger.debug("Print cancelled, resetting job_power to 0")
			self.print_job_power = 0.0
			self.print_job_started = False
			self._autostart_file = None
			return
		...
		if event == Events.PRINT_STARTED and self._settings.getFloat(["cost_rate"]) > 0:
			...
			self._tendasmartplug_logger.debug(payload.get("path", None))
			for plug in self._settings.get(["arrSmartplugs"]):
				...
				self._tendasmartplug_logger.debug(self.print_job_power)

		if event == Events.PRINT_STARTED and self.powerOffWhenIdle is True:
			if self._abort_timer is not None:
				self._abort_timer.cancel()
				self._abort_timer = None
				self._tendasmartplug_logger.debug("Power off aborted because starting new print.")
		...
		if event == Events.PRINT_DONE and self.print_job_started:
			self._tendasmartplug_logger.debug(payload)
			...
			self._tendasmartplug_logger.debug("hours: %s" % hours)
			power_used = self.print_job_power * hours
			self._tendasmartplug_logger.debug("power used: %s" % power_used)
			power_cost = power_used * self._settings.getFloat(["cost_rate"])
			self._tendasmartplug_logger.debug("power total cost: %s" % power_cost)
		...
		if event == Events.PRINT_DONE and len(self.power_off_queue) > 0:
			self._tendasmartplug_logger.debug("power_off_queue: {}".format(self.power_off_queue))
		...
		if self.powerOffWhenIdle is True and event == Events.MOVIE_RENDERING:
			self._tendasmartplug_logger.debug("Timelapse generation started: %s" % payload.get("movie_basename", ""))
			self._timelapse_active = True

		if self._timelapse_active and event == Events.MOVIE_DONE or event == Events.MOVIE_FAILED:
			self._tendasmartplug_logger.debug("Timelapse generation finished: %s. Return Code: %s" % (
			...
			if self._gcode_queued:
				for plug in self._settings.get(['arrSmartplugs']):
					if plug["gcodeCmdOn"] and plug["gcodeRunCmdOn"] != "":
						self._tendasmartplug_logger.debug("sending gcode commands to printer.")
						self._printer.commands(plug["gcodeRunCmdOn"].split("\n"))
				self._gcode_queued = False
			if self._autostart_file:
				self._tendasmartplug_logger.debug("printer connected starting print of %s" % self._autostart_file)
		...
		if event == Events.UPLOAD and self._settings.getBoolean(["event_on_upload_monitoring"]):
			if payload.get("print", False):  # implemented in OctoPrint version 1.4.1
				self._tendasmartplug_logger.debug("File uploaded: %s. Turning enabled plugs on." % payload.get("name", ""))
				self._tendasmartplug_logger.debug(payload)
				for plug in self._settings.get(['arrSmartplugs']):
					self._tendasmartplug_logger.debug(plug)
					if plug["event_on_upload"] is True and self._printer.is_closed_or_error():
						...
						response = self.turn_on(plug["ip"])
						if response["currentState"] == "on":
							self._tendasmartplug_logger.debug("power on successful for %s attempting connection in %s seconds" % (plug["ip"], plug.get("autoConnectDelay", "0")))
							self._plugin_manager.send_plugin_message(self._identifier, response)
	...
	def _idle_poweroff(self):
		...
		if (uptime() / 60) <= (self._settings.get_int(["idleTimeout"])):
			self._tendasmartplug_logger.debug("Just booted so wait for time sync.")
			self._tendasmartplug_logger.debug("uptime: {}, comparison: {}".format((uptime() / 60), (self._settings.get_int(["idleTimeout"]))))
			self._reset_idle_timer()
			return

		self._tendasmartplug_logger.debug("Idle timeout reached after %s minute(s). Turning heaters off prior to powering off plugs." % self.idleTimeout)
		if self._wait_for_heaters():
			self._tendasmartplug_logger.debug("Heaters below temperature.")
			if self._wait_for_timelapse():
				self._timer_start()
		else:
			self._tendasmartplug_logger.debug("Aborted power off due to activity.")

	##~~ Timelapse Monitoring

	def _wait_for_timelapse(self):
		self._waitForTimelapse = True
		self._tendasmartplug_logger.debug("Checking timelapse status before shutting off power...")
		...
		while True:
			...
			self._tendasmartplug_logger.debug("Waiting for timelapse before shutting off power...")
			time.sleep(5)
	...
	def _wait_for_heaters(self):
		...
		for heater, entry in heaters.items():
			...
			if temp != 0:
				self._tendasmartplug_logger.debug("Turning off heater: %s" % heater)
				self._skipIdleTimer = True
				self._printer.set_temperature(heater, 0)
				self._skipIdleTimer = False
			else:
				self._tendasmartplug_logger.debug("Heater %s already off." % heater)
		...
		while True:
			...
			for heater, entry in heaters.items():
				...
				self._tendasmartplug_logger.debug("Heater %s = %sC" % (heater, temp))
			...
			self._tendasmartplug_logger.debug("Waiting for heaters(%s) before shutting power off..." % ', '.join(heaters_above_waittemp))
			time.sleep(5)
	...
	def _timer_start(self):
		...
		self._tendasmartplug_logger.debug("Starting abort power off timer.")
	...
	def _shutdown_system(self):
		self._tendasmartplug_logger.debug("Automatically powering off enabled plugs.")
		for plug in self._settings.get(['arrSmartplugs']):
	...
	def _get_device_id(self, plugip):
		...
		if not response:
			...
			self._tendasmartplug_logger.debug(check_status_cmnd)
			...
			if response:
				...
				self._settings.save()
		self._tendasmartplug_logger.debug("get_device_id response: %s" % response)
		return response
	...
	def sendCommand(self, cmd, plugip, plug_num=0):
		if re.search('/\d+$', plugip):
			self._tendasmartplug_logger.exception("Internal error passing unsplit %s", plugip)
		...
		try:
			...
			r = requests.post(url=f"http://{headers['Host']}/{cmd}", headers=headers, json=json)
			if r.status_code != 200:
				self._tendasmartplug_logger.debug(f"Device responded with {r.status_code}")
				return r
		except Exception as e:
			self._tendasmartplug_logger.debug(f"Device failed to responded: {e}")

	##~~ Gcode processing hook

	def gcode_turn_off(self, plug):
		if self._printer.is_printing() and plug["warnPrinting"] is True:
			self._tendasmartplug_logger.debug("Not powering off %s immediately because printer is printing." % plug["label"])
	...
	def processGCODE(self, comm_instance, phase, cmd, cmd_type, gcode, *args, **kwargs):
		...
		if gcode == "M80":
			...
			self._tendasmartplug_logger.debug("Received M80 command, attempting power on of %s." % plugip)
			plug = self.plug_search(self._settings.get(["arrSmartplugs"]), "ip", plugip)
			self._tendasmartplug_logger.debug(plug)
		...
		if gcode == "M81":
			...
			self._tendasmartplug_logger.debug("Received M81 command, attempting power off of %s." % plugip)
			plug = self.plug_search(self._settings.get(["arrSmartplugs"]), "ip", plugip)
			self._tendasmartplug_logger.debug(plug)
			if plug and plug["gcodeEnabled"]:
	...
	def processAtCommand(self, comm_instance, phase, command, parameters, tags=None, *args, **kwargs):
		if command == "TENDAON":
			plugip = parameters
			self._tendasmartplug_logger.debug("Received TENDAON command, attempting power on of %s." % plugip)
			plug = self.plug_search(self._settings.get(["arrSmartplugs"]), "ip", plugip)
			self._tendasmartplug_logger.debug(plug)
			if plug and plug["gcodeEnabled"]:
				t = threading.Timer(int(plug["gcodeOnDelay"]), self.gcode_turn_on, [plug])
		...
		if command == "TENDAOFF":
			plugip = parameters
			self._tendasmartplug_logger.debug("Received TENDAOFF command, attempting power off of %s." % plugip)
			plug = self.plug_search(self._settings.get(["arrSmartplugs"]), "ip", plugip)
			self._tendasmartplug_logger.debug(plug)
			if plug and plug["gcodeEnabled"]:
				t = threading.Timer(int(plug["gcodeOffDelay"]), self.gcode_turn_off, [plug])
				...
				t.start()
			return None
		if command == 'TENDAIDON':
			self.powerOffWhenIdle = True
			self._reset_idle_timer()
		if command == 'TENDAIDOFF':
			self.powerOffWhenIdle = False
			self._stop_idle_timer()
			if self._abort_timer is not None:
				self._abort_timer.cancel()
				self._abort_timer = None
			self._timeout_value = None
		if command in ["TENDAIDON", "TENDAIDOFF"]:
			self._plugin_manager.send_plugin_message(self._identifier, dict(powerOffWhenIdle=self.powerOffWhenIdle, type="timeout", timeout_value=self._timeout_value))
	...
	def check_temps(self, parsed_temps):
		...
		for k, v in parsed_temps.items():
			if k == "B" and v[0] > int(self._settings.get(["thermal_runaway_max_bed"])):
				self._tendasmartplug_logger.debug("Max bed temp reached, shutting off plugs.")
				thermal_runaway_triggered = True
			if k.startswith("T") and v[0] > int(self._settings.get(["thermal_runaway_max_extruder"])):
				self._tendasmartplug_logger.debug("Extruder max temp reached, shutting off plugs.")
				thermal_runaway_triggered = True
	...
	def get_update_information(self):
		return dict(
			tendasmartplug=dict(
				displayName="Tenda Smartplug",
				displayVersion=self._plugin_version,
				type="github_release",
				user="Neodbaryd",
				repo="OctoPrint-tendasmartplug",
				...
				pip="https://github.com/Neodbaryd/OctoPrint-TendaSP3-Smartplug/archive/{target_version}.zip"
			)
		)


__plugin_name__ = "Tenda Smartplug"
__plugin_pythoncompat__ = ">=2.7,<4"


def __plugin_load__():
	global __plugin_implementation__
	__plugin_implementation__ = tendasmartplugPlugin()
